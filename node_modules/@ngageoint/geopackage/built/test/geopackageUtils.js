...
var GP = __importStar(require("../index"));
var wkb_1 = require("../lib/wkb");
var DataTypes = GP.DataTypes, GeometryColumns = GP.GeometryColumns, GeometryData = GP.GeometryData, BoundingBox = GP.BoundingBox, FeatureColumn = GP.FeatureColumn, DataColumns = GP.DataColumns, UserColumn = GP.UserColumn, Metadata = GP.Metadata, MetadataReference = GP.MetadataReference, RTreeIndex = GP.RTreeIndex, CrsWktExtension = GP.CrsWktExtension, SchemaExtension = GP.SchemaExtension, MetadataExtension = GP.MetadataExtension, WebPExtension = GP.WebPExtension, DataColumnsDao = GP.DataColumnsDao, DataColumnConstraintsDao = GP.DataColumnConstraintsDao, TableCreator = GP.TableCreator, MediaTable = GP.MediaTable, UserMappingTable = GP.UserMappingTable, DublinCoreType = GP.DublinCoreType, wkb = wkb_1.WKB;
var wkx = require('wkx'), path = require('path'), fs = require('fs-extra');
var GeoPackageUtils = {};
module.exports = GeoPackageUtils;
exports.default = GeoPackageUtils;
...
GeoPackageUtils.createFeatureTableAndAddFeatures = function (geopackage, tableName, features, type) {
    ...
    columns.push(FeatureColumn.createColumn(columnNumber++, 'text', DataTypes.TEXT, false, ''));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'real', DataTypes.REAL, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'boolean', DataTypes.BOOLEAN, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'blob', DataTypes.BLOB, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'integer', DataTypes.INTEGER, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'text_limited', DataTypes.TEXT, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'blob_limited', DataTypes.BLOB, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'date', DataTypes.DATE, false, null));
    columns.push(FeatureColumn.createColumn(columnNumber++, 'datetime', DataTypes.DATETIME, false, null));
    return geopackage
...
GeoPackageUtils.createFeature = function (geopackage, geoJson, name, featureDao) {
    ...
    featureRow.setValueWithColumnName('blob', Buffer.from(Math.random()
        .toString(36)
        .replace(/[^a-z]+/g, '')
        .substr(0, 5)));
    featureRow.setValueWithColumnName('integer', Math.round(Math.random() * 500));
    featureRow.setValueWithColumnName('text_limited', Math.random()
        .toString(36)
        .replace(/[^a-z]+/g, '')
        .substr(0, 5));
    featureRow.setValueWithColumnName('blob_limited', Buffer.from(Math.random()
        .toString(36)
        .replace(/[^a-z]+/g, '')
        .substr(0, 5)));
...
GeoPackageUtils.createSchemaExtension = function (geopackage) {
    ...
    return tc
        ...
        .then(function () {
        var dcd = geopackage.dataColumnConstraintsDao;
        ...
        var dc = geopackage.dataColumnsDao;
        ...
        for (var i = 0; i < featureTables.length; i++) {
            ...
            for (var c = 0; c < table.columns.length; c++) {
                ...
                if (column.primaryKey || column.getTypeName() !== 'INTEGER')
                    continue;
...
GeoPackageUtils.createRelatedTablesMediaExtension = function (geopackage) {
    ...
    return GeoPackageUtils.loadFile(path.join(__dirname, 'fixtures', 'BITSystems_Logo.png'))
        ...
        var ngaRowId = index_1.GeoPackage.addMedia(geopackage, 'media', ngaLogoBuffer, 'image/png');
        ...
        return rows.reduce(function (sequence, row) {
            return sequence.then(function () {
                var featureRow = featureDao.getRow(row);
                index_1.GeoPackage.linkMedia(geopackage, 'geometry2', featureRow.getId(), 'media', ngaRowId).then(function () {
                    var relationships = index_1.GeoPackage.getLinkedMedia(geopackage, 'geometry2', featureRow.getId());
...
GeoPackageUtils.createRelatedTablesFeaturesExtension = function (geopackage) {
    ...
    columns.push(UserColumn.createColumn(columnIndex++, DublinCoreType.DATE.name, DataTypes.DATETIME));
    columns.push(UserColumn.createColumn(columnIndex++, DublinCoreType.DESCRIPTION.name, DataTypes.TEXT));
    columns.push(UserColumn.createColumn(columnIndex++, DublinCoreType.SOURCE.name, DataTypes.TEXT));
    columns.push(UserColumn.createColumn(columnIndex++, DublinCoreType.TITLE.name, DataTypes.TEXT));
...
GeoPackageUtils.createTiles = function (geopackage) {
    return GeoPackageUtils.addWebMercatorTilesFromPath(geopackage, 'OSM', path.join(__dirname, 'fixtures', 'tiles'), 0, 3).then(function () {
        return geopackage;
    });
};
                    ...
                    return xSequence.then(function () {
                        ...
                        return yfilenames.reduce(function (ySequence, yFilename) {
                            return ySequence.then(function () {
                                ...
                                if (Number.isNaN(y))
                                    return;
                                return GeoPackageUtils.loadFile(path.join(__dirname, 'fixtures', 'tiles', zoom.toString(), x.toString(), y.toString() + '.png')).then(function (image) {
...
GeoPackageUtils.createWebPExtension = function (geopackage) {
    ...
    var tileMatrixSetBoundingBox = new BoundingBox(-11667347.997449303, 4824705.2253603265, -11666125.00499674, 4825928.217812888);
    var contentsBoundingBox = new BoundingBox(-11667347.997449303, 4824705.2253603265, -11666125.00499674, 4825928.217812888);
    var contentsSrsId = 3857;
    var tileMatrixSetSrsId = 3857;
    geopackage.spatialReferenceSystemDao.createWebMercator();
    return geopackage
...
GeoPackageUtils.createAttributes = function (geopackage) {
    ...
    columns.push(UserColumn.createColumn(columnNumber++, 'text', DataTypes.TEXT, false, ''));
    columns.push(UserColumn.createColumn(columnNumber++, 'real', DataTypes.REAL, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'boolean', DataTypes.BOOLEAN, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'blob', DataTypes.BLOB, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'integer', DataTypes.INTEGER, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'text_limited', DataTypes.TEXT, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'blob_limited', DataTypes.BLOB, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'date', DataTypes.DATE, false, null));
    columns.push(UserColumn.createColumn(columnNumber++, 'datetime', DataTypes.DATETIME, false, null));
    ...
    return geopackage.createAttributeTable(tableName, columns, [dc]).then(function (result) {
        ...
        for (var i = 0; i < 10; i++) {
            ...
            attributeRow.setValueWithColumnName('blob', Buffer.from(Math.random()
                .toString(36)
                .replace(/[^a-z]+/g, '')
                .substr(0, 5)));
            attributeRow.setValueWithColumnName('integer', Math.round(Math.random() * 500));
            attributeRow.setValueWithColumnName('text_limited', Math.random()
                .toString(36)
                .replace(/[^a-z]+/g, '')
                .substr(0, 5));
            attributeRow.setValueWithColumnName('blob_limited', Buffer.from(Math.random()
                .toString(36)
                .replace(/[^a-z]+/g, '')
                .substr(0, 5)));
        ...
        var row = {
            ...
            text_limited: Math.random()
                ...
                .replace(/[^a-z]+/g, '')
                .substr(0, 5),
            blob_limited: Buffer.from(Math.random()
                .toString(36)
                .replace(/[^a-z]+/g, '')
                .substr(0, 5)),
        ...
        index_1.GeoPackage.addAttributeRow(geopackage, tableName, row);
...
//# sourceMappingURL=geopackageUtils.js.map