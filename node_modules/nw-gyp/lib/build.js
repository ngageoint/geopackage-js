...
function build (gyp, argv, callback) {
  ...
  function loadConfigGypi () {
    fs.readFile(configPath, 'utf8', function (err, data) {
      if (err) {
        if (err.code == 'ENOENT') {
          callback(new Error('You must run `nw-gyp configure` first!'))
        } else {
          callback(err)
        }
        return
  ...
  function doBuild () {
    ...
    if (win) {
      ...
      argv.push('/nologo')
      if (config.variables.clang && config.variables.clang !== 'false') {
	log.verbose("Using clang-cl.exe and lld-link.exe.")
        argv.push('/p:CLToolExe=clang-cl.exe')
