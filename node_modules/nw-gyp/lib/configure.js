...
function configure (gyp, argv, callback) {
  ...
  function createConfigFile (err, vsSetup) {
    ...
    isV39 = semver.gt(release.version, '0.39.0-beta0')
    ...
    if (isV39 && win) {
      if (gyp.opts.clang === undefined) {
        variables.clang = true;
      } else {
	variables.clang = gyp.opts.clang;
      }
    }
    if (isLegacy && win) {
  ...
  function runGyp (err) {
    ...
    fs.stat(common_gypi, function (err, stat) {
      ...
      var nodeLibFile = path.join(nodeDir,
        ...
        'node.lib')
      var nwLibFile = path.join(nodeDir,
        !gyp.opts.nodedir ? '<(target_arch)' : '$(Configuration)',
        release.name + '.lib')
