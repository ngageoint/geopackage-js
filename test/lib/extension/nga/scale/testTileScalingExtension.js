  ...
  describe('Test Add Scaling Extension', function() {
    ...
    it('should get the x:13683, y: 24889, z: 16 tile', function(done) {
     ...
     try {
       ...
       gpr.getTile(13683,24889,16)
         .then(function(tile) {
           testSetup.diffImages(tile.getData(), path.join(__dirname, '..','..','..', '..', 'fixtures','tiles','16','13683', isWeb ? 'web' : '', isLinux ? '24889_linux.png' : '24889.png'), function(err, equal) {
             try {
    ...
    it('should get a scaled tile for x: 27366, y: 49778, z: 17 tile', function(done) {
      ...
      gpr.getTile(27366, 49778,17)
        .then(function(tile) {
          testSetup.diffImages(tile.getData(), path.join(__dirname, '..','..','..','..','fixtures','tiles','17','27366', isWeb ? 'web' : '', isLinux ? '49778_linux.png' : '49778.png'), function(err, equal) {
            try {
              equal.should.be.equal(true);
              done();
            } catch (e) {
