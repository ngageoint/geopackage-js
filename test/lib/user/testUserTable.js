...
describe('UserTable tests', function () {
  ...
  it('should create a UserCustomTable object', function () {
    ...
    userTable.addConstraint(
      new RawConstraint(
        ConstraintType.FOREIGN_KEY,
        'test_fk',
        'FOREIGN KEY (test_table_text) REFERENCES Persons(PersonID)',
      ),
    );
    userTable.addConstraint(
      new RawConstraint(
        ConstraintType.FOREIGN_KEY,
        'test_fk_2',
        'FOREIGN KEY (test_table_text2) REFERENCES Things(ThingID)',
      ),
    );
  ...
  it('should rename UserTable columns', function () {
    ...
    userTable.getTableName().should.be.equal('test_table');
    expect(() => {
      userTable.getColumn('test_table_string');
    }).to.throw();
    ...
    expect(() => {
      userTable.getColumn('test_table_text');
    }).to.throw();
  ...
  it('should add and drop UserTable columns', function () {
    userTable.addColumn(
      UserCustomColumn.createColumnWithIndex(2, 'test_table_integer', GeoPackageDataType.INTEGER, true, 5),
    );
    should.exist(userTable.getColumn('test_table_integer'));
    userTable.dropColumnWithName('test_table_integer');
    expect(() => {
      userTable.getColumn('test_table_integer');
    }).to.throw();
    userTable.addColumn(
      UserCustomColumn.createColumnWithIndex(2, 'test_table_integer', GeoPackageDataType.INTEGER, true, 5),
    );
