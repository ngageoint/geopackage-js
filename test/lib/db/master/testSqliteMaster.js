...
describe('SQLiteMaster tests', function () {
  ...
  it('should return the count for a given table name', function () {
    const count = SQLiteMaster.count(
      geoPackage.getConnection(),
      [],
      SQLiteMasterQuery.createForColumnValue(SQLiteMasterColumn.NAME, tableName),
    );
  ...
  it("should return the count for a TBL_NAME that is '=' to the table name", function () {
    const count = SQLiteMaster.count(
      geoPackage.getConnection(),
      [],
      SQLiteMasterQuery.createForOperationAndColumnValue(SQLiteMasterColumn.NAME, '=', tableName),
    );
    count.should.be.equal(1);
  });
  ...
  it('should run a single query against the SQLiteMaster table', function () {
    try {
      const sqliteMaster = SQLiteMaster.query(
        geoPackage.getConnection(),
  ...
  it('should run a combined query against the SQLiteMaster table', function () {
    ...
    const sqliteMaster = SQLiteMaster.query(
      geoPackage.getConnection(),
      [SQLiteMasterColumn.TBL_NAME, SQLiteMasterColumn.TYPE],
      [],
      sqliteMasterQuery,
    );
  ...
  it('should run a createOrForOperationAndColumnValue against the SQLiteMaster table', function () {
    const sqliteMasterQuery = SQLiteMasterQuery.createOrForOperationAndColumnValue(SQLiteMasterColumn.TBL_NAME, '=', [
      tableName,
    ]);
    const sqliteMaster = SQLiteMaster.query(
      geoPackage.getConnection(),
  ...
  it('should run a createAndForOperationAndColumnValue against the SQLiteMaster table', function () {
    const sqliteMasterQuery = SQLiteMasterQuery.createAndForOperationAndColumnValue(SQLiteMasterColumn.TBL_NAME, '=', [
      tableName,
    ]);
    const sqliteMaster = SQLiteMaster.query(
      geoPackage.getConnection(),
