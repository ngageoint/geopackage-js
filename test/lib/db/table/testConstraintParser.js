...
describe('Constraint Tests', function() {
  it('test table constraints', function() {
    testSQLScript(TableCreator.tableCreationScripts.spatial_reference_system, 0, 0, 0, 0, []);
    testSQLScript(TableCreator.tableCreationScripts.contents, 0, 0, 0, 1, ["fk_gc_r_srs_id"]);
    testSQLScript(TableCreator.tableCreationScripts.geometry_columns, 1, 1, 0, 2, ["pk_geom_cols", "uk_gc_table_name", "fk_gc_tn", "fk_gc_srs"]);
    testSQLScript(TableCreator.tableCreationScripts.tile_matrix_set, 0, 0, 0, 2, ["fk_gtms_table_name", "fk_gtms_srs"]);
    testSQLScript(TableCreator.tableCreationScripts.tile_matrix, 1, 0, 0, 1, ["pk_ttm", "fk_tmm_table_name"]);
    testSQLScript(TableCreator.tableCreationScripts.data_columns, 1, 1, 0, 0, ["pk_gdc", "gdc_tn"]);
    testSQLScript(TableCreator.tableCreationScripts.data_column_constraints, 0, 1, 0, 0, ["gdcc_ntv"]);
    testSQLScript(TableCreator.tableCreationScripts.metadata, 0, 0, 0, 0, ["m_pk"]);
    testSQLScript(TableCreator.tableCreationScripts.metadata_reference, 0, 0, 0, 2, ["crmr_mfi_fk", "crmr_mpi_fk"]);
    testSQLScript(TableCreator.tableCreationScripts.extensions, 0, 1, 0, 0, ["ge_tce"]);
    testSQLScript(TableCreator.tableCreationScripts.extended_relations, 0, 0, 0, 0, []);
    testSQLScript(TableCreator.tableCreationScripts.table_index, 0, 0, 0, 0, []);
    testSQLScript(TableCreator.tableCreationScripts.geometry_index, 1, 0, 0, 1, ["pk_ngi", "fk_ngi_nti_tn"]);
    testSQLScript(TableCreator.tableCreationScripts.feature_tile_link, 1, 0, 0, 0, ["pk_nftl"]);
    testSQLScript(TableCreator.tableCreationScripts.tile_scaling, 0, 0, 1, 1, ["fk_nts_gtms_tn", null]);
    testSQLScript(TableCreator.tableCreationScripts.contents_id, 0, 1, 0, 1, ["uk_nci_table_name", "fk_nci_gc_tn"]);
  });
});
