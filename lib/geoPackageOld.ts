...
// import { RelatedTablesExtension } from './extension/related';
// import { FeatureStyleExtension } from './extension/nga/style/featureStyleExtension';
// import { ContentsIdExtension } from './extension/nga/contents/contentsIdExtension';
// import { TileScalingExtension } from './extension/nga/scale/tileTableScaling';
...
// import { ExtendedRelationsDao } from './extension/related/extendedRelationDao';
...
// import { StyleMappingTable } from './extension/nga/style/styleMappingTable';
...
// import { GeoPackageTableCreator } from './db/tableCreator';
...
//   tableCreator: GeoPackageTableCreator;
...
//   private _extensionDao: ExtensionsDao;
...
//   private _extendedRelationDao: ExtendedRelationsDao;
//   private _contentsIdDao: ContentsIdDao;
//   private _tileScalingDao: TileScalingDao;
//   private _featureTileLinkDao: FeatureTileLinkDao;
...
//       try {
//         if (
//           spatialReferenceSystem.srs_id > 0 &&
//           (spatialReferenceSystem.organization !== ProjectionConstants.EPSG ||
...
//             [spatialReferenceSystem.organization, spatialReferenceSystem.organization_coordsys_id].join(':'),
//             spatialReferenceSystem.definition,
//           );
//       } catch (e) {}
//     });
//   }
...
//     return this._tileMatrixDao || (this._tileMatrixDao = new TileMatrixDao(this.connection));
//   }
//   get dataColumnsDao(): DataColumnsDao {
//     return this._dataColumnsDao || (this._dataColumnsDao = new DataColumnsDao(this));
//   }
//   get extensionDao(): ExtensionsDao {
//     return this._extensionDao || (this._extensionDao = new ExtensionsDao(this));
//   }
...
//   }
//   get extendedRelationDao(): ExtendedRelationsDao {
//     return this._extendedRelationDao || (this._extendedRelationDao = new ExtendedRelationsDao(this));
//   }
...
//   }
//   get featureTileLinkDao(): FeatureTileLinkDao {
//     return this._featureTileLinkDao || (this._featureTileLinkDao = new FeatureTileLinkDao(this));
//   }
...
//     return this.connection.viewExists(view);
//   }
//
//   /**
//    * checks if table or view exists
//    * @param name
//    */
//   public isTableOrView(name: string): boolean {
//     return this.connection.tableOrViewExists(name);
//   }
//
//   /**
...
//     new ExtensionManager(this).deleteTableExtensions(table);
...
//   }
//
//   getTableCreator(): GeoPackageTableCreator {
...
//     }[],
//   ): boolean {
//     const attributeColumns: AttributesColumn[] = [];
//     let columnNumber = 0;
//     const dataColumns = [];
...
//     index = false,
//     batchSize = 1000,
//     progress?: (featuresAdded: number) => void,
//   ): Promise<number> {
//     let inserted = 0;
//     const featureDao = this.getFeatureDao(tableName);
...
//     const reprojectionNeeded = !(
//       srs.organization === ProjectionConstants.EPSG &&
...
//             const id = featureDao.connection.adapter.bindAndInsert(
//               insertStatement,
//               SqliteQueryBuilder.buildUpdateOrInsertObject(featureRow),
//             );
//             inserted++;
...
//   }
//   createFeatureTableFromProperties(tableName: string, properties: { name: string; dataType: string }[]): boolean {
...
//       columns.push(
//         FeatureColumn.createColumn(columnNumber++, property.name, GeoPackageDataType.fromName(property.dataType)),
//       );
//     }
...
//     Projection.loadProjection(
//       [spatialReferenceSystem.organization.toUpperCase(), spatialReferenceSystem.organization_coordsys_id].join(':'),
//       spatialReferenceSystem.definition,
//     );
...
//     tileSize = 256,
//   ): TileMatrixSet {
//     let webMercator = this.spatialReferenceSystemDao.getByOrganizationAndCoordSysId(
//       ProjectionConstants.EPSG,
//       ProjectionConstants.EPSG_CODE_3857,
//     );
//     if (!webMercator) {
//       this.spatialReferenceSystemDao.createWebMercator();
//       webMercator = this.spatialReferenceSystemDao.getByOrganizationAndCoordSysId(
//         ProjectionConstants.EPSG,
//         ProjectionConstants.EPSG_CODE_3857,
//       );
//     }
...
//       const srsDao = new SpatialReferenceSystemDao(this);
//       const from = srsDao.getBySrsId(contentsSrsId).projection;
//       contentsBoundingBox = contentsBoundingBox.projectBoundingBox(from, ProjectionConstants.EPSG_WEB_MERCATOR);
//     }
//     if (tileMatrixSetSrsId !== webMercatorSrsId) {
//       const srsDao = new SpatialReferenceSystemDao(this);
//       const from = srsDao.getBySrsId(tileMatrixSetSrsId).projection;
//       tileMatrixSetBoundingBox = tileMatrixSetBoundingBox.projectBoundingBox(
//         from,
...
//     boundingBox = boundingBox.projectBoundingBox(ProjectionConstants.EPSG_WEB_MERCATOR, ProjectionConstants.EPSG_4326);
//
//     if (tileCount > 10000) {
...
//       let geoJsonGeom = geometry.getGeometry().toGeoJSON() as Geometry;
//       if (
//         srs.definition &&
...
//     const bb = webMercatorBoundingBox.projectBoundingBox(
//       ProjectionConstants.EPSG_WEB_MERCATOR,
//       ProjectionConstants.EPSG_4326,
//     );
...
//   }
//
//   renameTable(tableName, newTableName): void {
...
//   }
//
//   copyTableAndExtensions(tableName, newTableName): void {
//     this.copyTable(tableName, newTableName, true, true);
//   }
//
//   copyTableNoExtensions(tableName, newTableName): void {
//     this.copyTable(tableName, newTableName, true, false);
//   }
//
//   copyTableAsEmpty(tableName, newTableName): void {
//     this.copyTable(tableName, newTableName, false, false);
//   }
//
//   getTableDataType(tableName): ContentsDataType {
//     let tableType = null;
...
//    */
//   copyTable(tableName, newTableName, transferContent, extensions): void {
//     const dataType = this.getTableDataType(tableName);
//     if (dataType !== null && dataType !== undefined) {
//       if (dataType === ContentsDataType.ATTRIBUTES) {
//         this.copyAttributeTable(tableName, newTableName, transferContent);
//       } else if (dataType === ContentsDataType.FEATURES) {
//         this.copyFeatureTable(tableName, newTableName, transferContent);
//       } else if (dataType === ContentsDataType.TILES) {
...
//     // Copy extensions
//     if (extensions) {
//       new ExtensionManager(this).copyTableExtensions(tableName, newTableName);
//     }
//   }
//
//   /**
...
//    * @param transferContent transfer content flag
//    */
//   copyAttributeTable(tableName, newTableName, transferContent): void {
...
//    * @param transferContent transfer content flag
//    */
//   copyFeatureTable(tableName, newTableName, transferContent): void {
...
//    * @param transferContent transfer content flag
//    */
//   copyTileTable(tableName, newTableName, transferContent): void {
...
//     try {
//       tileMatrices = tileMatrixDao
//         .queryForAllEq(TileMatrixDao.COLUMN_TABLE_NAME, tableName)
//         .map(results => tileMatrixDao.createObject(results));
//     } catch (e) {
...
//    */
//   copyUserTable(tableName, newTableName, transferContent, validateContents = true): Contents {
