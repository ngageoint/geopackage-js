...
UserDao.prototype.linkRelatedRow = function(userRow, relatedRow, relationType, mappingTableName) {
  ...
  var mappingTableName = mappingTableName || baseTableName + '_' + relatedTableName;
...
UserDao.prototype.linkFeatureRow = function(userRow, featureRow, mappingTableName) {
  return this.linkRelatedRow(userRow, featureRow, RelationType.FEATURES, mappingTableName);
}

UserDao.prototype.linkMediaRow = function(userRow, mediaRow, mappingTableName) {
  return this.linkRelatedRow(userRow, mediaRow, RelationType.MEDIA, mappingTableName);
}

UserDao.prototype.linkSimpleAttributesRow = function(userRow, simpleAttrbuteRow, mappingTableName) {
  return this.linkRelatedRow(userRow, simpleAttrbuteRow, RelationType.SIMPLE_ATTRIBUTES, mappingTableName);
}
...
UserDao.prototype.getLinkedMedia = function(userRow) {
  ...
  for (var i = 0; i < mediaRelations.length; i++) {
    ...
    for (var m = 0; m < mappings.length; m++) {
      ...
      linkedMedia.push(mediaDao.queryForIdObject(relatedId));
    }
  }
  return linkedMedia;
}
...
UserDao.prototype.getLinkedSimpleAttributes = function(userRow) {
  ...
  for (var i = 0; i < simpleRelations.length; i++) {
    ...
    for (var m = 0; m < mappings.length; m++) {
      var relatedId = mappings[m].related_id;
      linkedSimpleAttributes.push(simpleDao.queryForIdObject(relatedId));
    }
  }
...
UserDao.prototype.getLinkedFeatures = function(userRow) {
  ...
  for (var i = 0; i < featureRelations.length; i++) {
    ...
    var featureDao = this.geoPackage.getFeatureDaoWithTableName(featureRelation.base_table_name);
    var userMappingDao = rte.getMappingDao(featureRelation.mapping_table_name);
    var mappings = userMappingDao.queryByBaseId(userRow.getId());
    for (var m = 0; m < mappings.length; m++) {
      var relatedId = mappings[m].related_id;
      linkedFeatures.push(featureDao.queryForIdObject(relatedId));
    }
  }
