...
Dao.prototype.update = function(object, callback) {
  ...
  if (object.getColumnNames) {
    var columnNames = object.getColumnNames();
    for (var i = 0; i < columnNames.length; i++) {
      updateValues['$'+columnNames[i].replace('\.', '_')] = object.toDatabaseValue(columnNames[i]);
    }
  } else {
    for (var key in object) {
      if (object.hasOwnProperty(key)) {
        if (object.toDatabaseValue) {
          updateValues['$' + key.replace('\.', '_')] = object.toDatabaseValue(key);
        } else {
          updateValues['$' + key.replace('\.', '_')] = object[key];
        }
      }
    }
  }
  ...
  for (var i = 0; i < multiId.length; i++) {
    where += '"' + this.idColumns[i] + '" = $' + this.idColumns[i].replace('\.', '_') +'';
    updateValues['$' + this.idColumns[i].replace('\.', '_') +''] = multiId[i];
  }
