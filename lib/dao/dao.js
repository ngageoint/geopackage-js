...
Dao.prototype.queryJoinWhereWithArgs = function(join, where, whereArgs, columns, rowCallback, doneCallback) {
...
Dao.prototype.create = function(object, callback) {
  ...
  if (object.getColumnNames) {
    ...
    for (var i = 0; i < columnNames.length; i++) {
      insertObject['$'+columnNames[i].replace(/\.|\s/g, '_')] = object.toDatabaseValue(columnNames[i]);
    }
  } else {
    for (var key in object) {
      if (object.hasOwnProperty(key)) {
        if (object.toDatabaseValue) {
          insertObject['$' + key.replace(/\.|\s/g, '_')] = object.toDatabaseValue(key);
        } else {
          insertObject['$' + key.replace(/\.|\s/g, '_')] = object[key];
        }
      }
    }
  }
...
Dao.prototype.update = function(object, callback) {
  var updateValues = {};
  if (object.getColumnNames) {
    var columnNames = object.getColumnNames();
    for (var i = 0; i < columnNames.length; i++) {
      updateValues['$'+columnNames[i].replace(/\.|\s/g, '_')] = object.toDatabaseValue(columnNames[i]);
    }
  } else {
    for (var key in object) {
      if (object.hasOwnProperty(key)) {
        if (object.toDatabaseValue) {
          updateValues['$' + key.replace(/\.|\s/g, '_')] = object.toDatabaseValue(key);
        } else {
          updateValues['$' + key.replace(/\.|\s/g, '_')] = object[key];
        }
      }
    }
  }
  ...
  for (var i = 0; i < multiId.length; i++) {
    where += '"' + this.idColumns[i] + '" = $' + this.idColumns[i].replace(/\.|\s/g, '_');
    updateValues['$' + this.idColumns[i].replace(/\.|\s/g, '_')] = multiId[i];
  }
  ...
  update.sql += where;
  this.connection.run(update, updateValues, callback);
};

Dao.prototype.createOrUpdate = function(object, callback) {
  this.queryForSameId(object, function(err, existing) {
    if (!existing) {
