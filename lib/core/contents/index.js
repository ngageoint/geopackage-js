...
ContentsDao.prototype.getTables = function(tableType) {
  ...
  if (tableType) {
    ...
    results = this.queryForColumnsWhere('table_name', fieldValues);
  } else {
    results = this.queryForColumnsInAll('table_name');
  }
  var tableNames = [];
...
ContentsDao.prototype.getGeometryColumns = function (contents) {
  ...
  var results = dao.queryForAllEqWithFieldAndValue(GeometryColumnsDao.COLUMN_TABLE_NAME, contents.table_name);
...
ContentsDao.prototype.getTileMatrixSet = function (contents) {
  ...
  var results = dao.queryForAllEqWithFieldAndValue(TileMatrixSetDao.COLUMN_TABLE_NAME, contents.table_name);
  if (!results || !results.length) return;
  var gc = dao.createObject();
  dao.populateObjectFromResult(gc, results[0]);
  return gc;
...
ContentsDao.prototype.getTileMatrix = function (contents) {
  ...
  var results = dao.queryForAllEqWithFieldAndValue(TileMatrixDao.COLUMN_TABLE_NAME, contents.table_name);
  if (!results || !results.length) return;
  var tileMatricies = [];
  for (var i = 0; i < results.length; i++) {
    var gc = dao.createObject();
