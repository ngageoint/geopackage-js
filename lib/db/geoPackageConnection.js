...
var GeoPackageConnection = function(filePath, callback) {
  if (typeof(process) !== 'undefined' && process.version) {
    console.log('Using SQLite3 Node Process version ' + process.version);
    this.adapterCreator = require('./sqliteAdapter');
  } else {
    console.log('Outside of Node - Using sql.js');
...
GeoPackageConnection.prototype.setApplicationId = function(callback) {
  ...
  this.adapter.run('PRAGMA application_id = ' + applicationId, function() {
    this.adapter.run('PRAGMA user_version = ' + GeoPackageConstants.USER_VERSION, callback);
  }.bind(this));
}
