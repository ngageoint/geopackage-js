...
module.exports.determinePositionAndScale = function(geoPackageTileBoundingBox, tileHeight, tileWidth, totalBoundingBox, totalHeight, totalWidth) {
  ...
  p.dx = Math.round(xPositionInFinalTileUnits);
  p.dy = Math.round(yPositionInFinalTileUnits);
  p.sx = 0;
  p.sy = 0;
  p.dWidth = Math.round(widthInFinalTileUnits);
  p.dHeight = Math.round(heightInFinalTileUnits);
  p.sWidth = tileWidth;
