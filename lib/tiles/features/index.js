...
FeatureTiles.prototype.drawGeometry = async function(simplifyTolerance, geoJson, context, transform, boundingBox, featureStyle) {
  ...
  if (geoJson.type === 'Point') {
    ...
    for (i = 0; i < geoJson.coordinates.length; i++) {
      ...
      await this.drawPoint(ptGeom, context, boundingBox, featureStyle, transform);
    }
    ...
    for (i = 0; i < geoJson.coordinates.length; i++) {
      ...
      this.drawLine(simplifyTolerance, lsGeom, context, featureStyle, transform, boundingBox);
    }
