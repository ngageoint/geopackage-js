...
FeatureTiles.prototype.drawTileQueryIndex = function(x, y, z, tileCanvas) {
  ...
  if (this.maxFeaturesPerTile === null || tileCount <= this.maxFeaturesPerTile) {
    ...
    for (var gj of geojsonFeatures) {
      ...
      this.addFeatureToBatch(gj, context, drawProjection, boundingBox, style);
    }
  } else if (this.maxFeaturesTileDraw !== null) {
    // Draw the max features tile
    canvas = this.maxFeaturesTileDraw.drawTile(width, height, tileCount);
  }
 ...
 */
FeatureTiles.prototype.drawPoint = function(path, geoJson, context, boundingBox, featureStyle, drawProjection) {
  var width;
  var height;
  var iconX;
  var iconY;
  ...
  if (featureStyle != null && featureStyle.hasIcon()) {
    ...
    var image = iconRow.getDataImage(iconRow);
    width = this.scale * iconRow.getWidth();
    height = this.scale * iconRow.getHeight();
    ...
    if (x >= 0 - radius && x <= this.tileWidth + radius && y >= 0 - radius && y <= this.tileHeight + radius) {
      var circleX = Math.round(x - radius);
      var circleY = Math.round(y - radius);
      // Draw the circle border
      if (this.circleBorderColor != null) {
        context.fillStyle = pointPaint.getColorRGBA();
        context.beginPath();
        context.arc(circleX, circleY, radius + pointPaint.getStrokeWidth(), 0, 2 * Math.PI, true);
        context.fill();
        context.closePath()
      }
      // Draw the circle
 ...
 * @param featureStyle
 */
FeatureTiles.prototype.addFeatureToBatch = function(geoJson, context, drawProjection, boundingBox, featureStyle) {
  var path = new d3geo.geoPath()
    .context(context)
    .projection(drawProjection);
    if (geoJson.type === 'Point') {
      this.drawPoint(path, geoJson, context, boundingBox, featureStyle, drawProjection);
      ...
      geoJson.coordinates.forEach(c => {
        ...
        this.drawPoint(path, ptGeom, context, boundingBox, featureStyle, drawProjection);
      });
