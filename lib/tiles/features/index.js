...
FeatureTiles.prototype.drawTileQueryIndex = async function(x, y, z, tileCanvas) {
  ...
  if (this.maxFeaturesPerTile === null || tileCount <= this.maxFeaturesPerTile) {
    ...
    for (var featureRow of iterator) {
      ...
      await this.drawGeometry(simplifyTolerance, geojson, context, drawProjection, boundingBox, style);
...
FeatureTiles.prototype.drawTileWithBoundingBox = async function(boundingBox, zoom, tileCanvas) {
  ...
  for (var fr of featureRows) {
    ...
    await this.drawGeometry(simplifyTolerance, gj, context, drawProjection, boundingBox, style);
  }
  return new Promise(function(resolve, reject) {
    if (this.useNodeCanvas) {
      var writeStream = concat(function (buffer) {
 ...
 */
FeatureTiles.prototype.drawPoint = async function(geoJson, context, boundingBox, featureStyle, drawProjection) {
  var width;
  var height;
  var iconX;
  var iconY;

  const wgs84GeoJson = reproject.toWgs84(geoJson, this.projection);
  var transformedCoords = drawProjection([wgs84GeoJson.coordinates[0], wgs84GeoJson.coordinates[1]]);
 ...
 * @param context
 */
FeatureTiles.prototype.getPath = function(simplifyTolerance, lineString, transform, context) {
  // Simplify and Reproject line string
  const simplifiedLineString = reproject.toWgs84(this.simplifyPoints(simplifyTolerance, lineString), this.projection);
  if (simplifiedLineString.coordinates.length > 0) {
    var coordinate = simplifiedLineString.coordinates[0]
    var transformedCoords = transform([coordinate[0], coordinate[1]]);
    var x = transformedCoords[0];
    var y = transformedCoords[1];
    context.moveTo(x, y)
    for (var i = 1; i < simplifiedLineString.coordinates.length; i++) {
      coordinate = simplifiedLineString.coordinates[i]
      transformedCoords = transform([coordinate[0], coordinate[1]]);
 ...
 * @param transform
 */
FeatureTiles.prototype.drawLine = function(simplifyTolerance, geoJson, context, featureStyle, transform) {
  context.save();
  context.beginPath();
  var paint = this.getLinePaint(featureStyle);
  context.strokeStyle = paint.getColorRGBA();
  context.lineWidth = paint.getStrokeWidth();
  this.getPath(simplifyTolerance, geoJson, transform, context);
  context.stroke();
  context.closePath();
  context.restore();
};
 ...
 * @param transform
 */
FeatureTiles.prototype.drawPolygon = function(simplifyTolerance, geoJson, context, featureStyle, transform) {
  context.save();
  context.beginPath();
  this.getPath(simplifyTolerance, geoJson, transform, context);
  context.closePath();
 ...
 * @param featureStyle
 */
FeatureTiles.prototype.drawGeometry = async function(simplifyTolerance, geoJson, context, drawProjection, boundingBox, featureStyle) {
  var i, j, c, lsGeom, converted;
  if (geoJson.type === 'Point') {
    await this.drawPoint(geoJson, context, boundingBox, featureStyle, drawProjection);
  } else if (geoJson.type === 'LineString') {
    this.drawLine(simplifyTolerance, geoJson, context, featureStyle, drawProjection);
    ...
    for (i = 0; i < geoJson.coordinates.length; i++) {
      ...
      await this.drawPoint(ptGeom, context, boundingBox, featureStyle, drawProjection);
    }
    ...
    for (i = 0; i < geoJson.coordinates.length; i++) {
      ...
      this.drawLine(simplifyTolerance, lsGeom, context, featureStyle, drawProjection);
    }
  } else if (geoJson.type === 'Polygon') {
    converted = PolyToLine(geoJson)
    if (converted.geometry.type === 'LineString') {
      this.drawPolygon(simplifyTolerance, converted.geometry, context, featureStyle, drawProjection);
      ...
      for (i = 0; i < converted.geometry.coordinates.length; i++) {
        ...
        this.drawPolygon(simplifyTolerance, lsGeom, context, featureStyle, drawProjection);
      }
    }
    ...
    for (i = 0; i < geoJson.coordinates.length; i++) {
      ...
      converted = PolyToLine(pGeom)
      if (converted.geometry.type === 'LineString') {
        this.drawPolygon(simplifyTolerance, converted.geometry, context, featureStyle, drawProjection);
      } else if (converted.geometry.type === 'MultiLineString') {
        for (j = 0; j < converted.geometry.coordinates.length; j++) {
          lsGeom = {
            type: 'LineString',
            coordinates: converted.geometry.coordinates[j]
          };
          this.drawPolygon(simplifyTolerance, lsGeom, context, featureStyle, drawProjection);
        }
      }
