...
var PureImage = require('pureimage')
  , CustomFeatureTyle = require('./customFeaturesTile')
  , {registerFont, measureText} = require('pureimage/src/text')
...
var NumberFeaturesTile = function() {
  CustomFeatureTyle.call(this);
  this.textSize = 18;
  this.textFont = 'PTMono';
...
util.inherits(NumberFeaturesTile, CustomFeatureTyle);


/**
 * register a font
 * @param callback to be called once font has been successfully loaded
 * @private
 */
NumberFeaturesTile.prototype._registerDefaultFont = function(callback) {
  if (!this.defaultFontRegistered) {
    registerFont('lib/fonts/PTMono-Regular.ttf', 'PTMono').load(function () {
 ...
 * @return {Booleam} true if drawing unindexed tiles
 */
 ...
 * @returns {Bitmap}
 */
NumberFeaturesTile.prototype.drawUnindexedTile = function(tileWidth, tileHeight) {
  var image = null;
 ...
 */
NumberFeaturesTile.prototype.drawTile = function(tileWidth, tileHeight, text) {
  return new Promise(function(resolve, reject) {
    this._registerDefaultFont(function () {
      var image = PureImage.make(tileWidth, tileHeight);
      var context = image.getContext('2d');
      context.clearRect(0, 0, tileWidth, tileHeight);
      // Draw the tile border
      if (this.tileBorderColor !== null) {
        context.fillStyle = this.tileBorderColor;
        context.fillRect(0, 0, tileWidth, tileHeight);
      }
      var offset  = this.tileBorderStrokeWidth;
      var borderWidth = (offset * 2);
      var rectWidth = tileWidth - borderWidth;
      var rectHeight = tileHeight - borderWidth;
      // Draw the tile fill paint
      if (this.tileFillColor !== null) {
        context.fillStyle = this.tileFillColor;
        context.clearRect(offset, offset, rectWidth, rectHeight);
        context.fillRect(offset, offset, rectWidth, rectHeight);
      } else {
        context.clearRect(offset, offset, rectWidth, rectHeight);
      }
      context.font = this.textSize + 'pt \'' + this.textFont + '\'';
      var textSize = measureText(context, text);
      var textWidth = Math.round(textSize.width);
      var textHeight = Math.round(textSize.emHeightAscent);
      ...
      if (this.circleBorderColor != null || this.circleFillColor != null) {
        ...
        if (this.circleBorderColor != null) {
          context.fillStyle = this.circleBorderColor;
          context.beginPath();
          context.arc(centerX, centerY, radius + this.circleStrokeWidth, 0, 2 * Math.PI, true);
          context.fill_noaa();
          context.closePath()
        }
        // Draw the circle
      ...
      var textY = centerY + Math.round(textHeight / 2.0);
