...
function CanvasTileCreator(width, height, tileMatrix, tileMatrixSet, tileBoundingBox, projectionFrom, projectionTo, canvas, callback) {
  ...
  // this.worker = work(require('./tileWorker.js'));
...
CanvasTileCreator.prototype.reproject = function (tileData, tilePieceBoundingBox, callback) {
  ...
  worker.onmessage = function(e) {
    if (e.data.message === 'done') {
      ...
      var endY = e.data.endY;

      var tmpCanvas = document.createElement('canvas');
      tmpCanvas.width = width - offsetX;
      tmpCanvas.height = height - offsetY;
      tmpCanvas.getContext('2d').putImageData(new ImageData(imageData, width - offsetX, height - offsetY), 0, 0);
