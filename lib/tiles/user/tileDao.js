...
TileDao.prototype.queryForTile = function (column, row, zoomLevel) {
  ...
  for (var row of this.queryForFieldValues(fieldValues)) {
    tileRow = this.getTileRow(row);
  }
  return tileRow;
};

TileDao.prototype.queryForTilesWithZoomLevel = function (zoomLevel, tileCallback) {
  var iterator = this.queryForEachEqWithFieldAndValue(TileColumn.COLUMN_ZOOM_LEVEL, zoomLevel);
  var thisGetTileRow = this.getTileRow.bind(this);
  return {
    [Symbol.iterator]() {
      return {
        next: function() {
          ...
          if (!nextRow.done) {
            return {
              value: thisGetTileRow(nextRow.value),
              done: false
            };
          }
          return {
...
TileDao.prototype.queryForTilesDescending = function (zoomLevel, tileCallback) {
  var iterator = this.queryForEachEqWithField(TileColumn.COLUMN_ZOOM_LEVEL, zoomLevel, undefined, undefined, TileColumn.COLUMN_TILE_COLUMN + ' DESC, ' + TileColumn.COLUMN_TILE_ROW + ', DESC');
    ...
    [Symbol.iterator]() {
      return {
        next: function() {
          var nextRow = iterator.next();
          if (!nextRow.done) {
            var tileRow = thisGetTileRow(nextRow.value);
