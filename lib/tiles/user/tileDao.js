...
TileDao.prototype.queryForTile = function (column, row, zoomLevel) {
  ...
  var tileRow;
  return this.queryForFieldValues(fieldValues, function(err, result) {
...
TileDao.prototype.queryForTilesWithZoomLevel = function (zoomLevel, tileCallback) {
  return this.queryForEachEqWithFieldAndValue(TileColumn.COLUMN_ZOOM_LEVEL, zoomLevel, function(err, result) {
...
TileDao.prototype.queryForTilesDescending = function (zoomLevel, tileCallback) {
  return this.queryForEachEqWithField(TileColumn.COLUMN_ZOOM_LEVEL, zoomLevel, undefined, undefined, TileColumn.COLUMN_TILE_COLUMN + ' DESC, ' + TileColumn.COLUMN_TILE_ROW + ', DESC', function(err, result) {
    if(!tileCallback) return;
    if (err || !result) return tileCallback(err);
    tileCallback(err, this.getTileRow(result));
  }.bind(this));
 ...
 * @param  {Function} doneCallback called when all tiles have been retrieved
 */
TileDao.prototype.queryByTileGrid = function (tileGrid, zoomLevel, tileCallback) {
  ...
  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(TileColumn.COLUMN_TILE_COLUMN, tileGrid.min_x, '>=');
  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(TileColumn.COLUMN_TILE_COLUMN, tileGrid.max_x, '<=');
  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(TileColumn.COLUMN_TILE_ROW, tileGrid.min_y, '>=');
  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(TileColumn.COLUMN_TILE_ROW, tileGrid.max_y, '<=');
  var whereArgs = this.buildWhereArgsWithValueArray([zoomLevel, tileGrid.min_x, tileGrid.max_x, tileGrid.min_y, tileGrid.max_y]);

  return this.queryWhereWithArgsDistinct(where, whereArgs, function(err, result) {
    if(!tileCallback) return;
    if (!result) return;
    tileCallback(err, this.getTileRow(result));
  }.bind(this));
...
TileDao.prototype.getSrs = function() {
  return this.getContentsDao().getSrs(this.tileMatrixSet);
};
