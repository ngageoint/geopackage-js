...
FeatureTableIndex.prototype.getFeatureTableIndexExtension = function (callback) {
  this.extensionsDao.isTableExists(function(err, exists) {
    if (exists) {
      var extension;
      this.extensionsDao.queryByExtensionAndTableNameAndColumnName(this.extensionName, this.tableName, this.columnName, function(err, row, rowDone) {
        extension = row;
        rowDone();
      }, function() {
      ...
      this.extensionsDao.createTable(function(err) {
        if (err) callback(err);
        var extension;
        this.extensionsDao.queryByExtensionAndTableNameAndColumnName(this.extensionName, this.tableName, this.columnName, function(err, row, rowDone) {
          extension = row;
...
FeatureTableIndex.prototype.indexTable = function(tableIndex, progressCallback, callback) {
  ...
  this.featureDao.queryForEach(function(err, row, rowDone) {
    var fr = this.featureDao.getFeatureRow(row);
    this.indexRow(tableIndex, fr.getId(), fr.getGeometry(), rowDone);
...
FeatureTableIndex.prototype.queryWithGeometryEnvelope = function(envelope, rowCallback, doneCallback) {
  ...
  var featureTableIdColumn = this.featureDao.table.getPkColumn().name;
  var join = 'inner join ' + this.tableName + ' on ' + this.tableName + '.' + featureTableIdColumn + ' = ' + GeometryIndexDao.COLUMN_GEOM_ID;
