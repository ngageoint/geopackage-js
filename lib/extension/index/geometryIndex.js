 ...
 */
var GeometryIndexDao = function(connection, featureDao) {
...
GeometryIndexDao.prototype.getTableIndex = function(geometryIndex) {
  ...
  return dao.queryForIdObject(geometryIndex.tableName);
};
...
GeometryIndexDao.prototype.queryForTableName = function(tableName) {
  return this.queryForEachEqWithFieldAndValue(GeometryIndexDao.COLUMN_TABLE_NAME, tableName);
};
...
GeometryIndexDao.prototype.queryWithGeometryEnvelope = function(envelope) {
  ...
  if (minXLessThanMaxX) {
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_X, envelope.maxX, '<=');
    where += ' and ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_X, envelope.minX, '>=');
  } else {
    where += '(';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_X, envelope.maxX, '<=');
    where += ' or ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_X, envelope.minX, '>=');
    where += ' or ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_X, envelope.minX, '>=');
    where += ' or ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_X, envelope.maxX, '<=');
    where += ')';
  }

  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_Y, envelope.maxY, '<=');
  where += ' and ';
  where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_Y, envelope.minY, '>=');
  ...
  if (envelope.hasZ) {
    where += ' and ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_Z, envelope.minZ, '<=');
    where += ' and ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_Z, envelope.maxZ, '>=');
    whereArgs.push(envelope.maxZ, envelope.minZ);
  }

  if (envelope.hasM) {
    where += ' and ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MIN_M, envelope.minM, '<=');
    where += ' and ';
    where += this.buildWhereWithFieldAndValueAndOperation(GeometryIndexDao.COLUMN_MAX_M, envelope.maxM, '>=');
    whereArgs.push(envelope.maxM, envelope.minM);
  }

  var join = 'inner join ' + tableName + ' on ' + tableName + '.id = ' + GeometryIndexDao.COLUMN_GEOM_ID;
