  ...
  tileMatrixDao.queryForEqWithField(TileMatrixDao.COLUMN_TABLE_NAME, tileMatrixSet.table_name, null, null, TileMatrixDao.COLUMN_ZOOM_LEVEL + ' ASC, ' + TileMatrixDao.COLUMN_PIXEL_X_SIZE + ' DESC, ' + TileMatrixDao.COLUMN_PIXEL_Y_SIZE + ' DESC', function(err, results) {
    async.eachSeries(results, function(result, callback) {
      ...
      tableReader.readTileTable(this.connection, function(err, tileTable) {
        callback(null, new TileDao(this.connection, tileTable, tileMatrixSet, tileMatrices));
      }.bind(this));
    }.bind(this));
  }.bind(this));
};
 ...
 */
GeoPackage.prototype.getTables = function(callback) {
  var tables = {};
  this.getFeatureTables(function(err, featureTables) {
    tables.features = featureTables;
    this.getTileTables(function(err, tileTables) {
 ...
 */
GeoPackage.prototype.hasTileTable = function(tileTableName, callback) {
  this.getTileTables(function(err, tables) {
 ...
 * @param  {Function} callback   called with an error if one occurred and true or false for the existence of the table
 */
GeoPackage.prototype.hasFeatureTable = function(featureTableName, callback) {
  this.getFeatureTables(function(err, tables) {
...
GeoPackage.prototype.index = function(callback) {
  this.getFeatureTables(function(err, tables) {
    async.eachSeries(tables, function(table, tableDone) {
      this.indexFeatureTable(table, tableDone);
...
GeoPackage.prototype.indexFeatureTable = function(table, callback) {
  this.getFeatureDaoWithTableName(table, function(err, featureDao) {
...
GeoPackage.prototype.getFeatureDaoWithTableName = function (tableName, callback) {
  ...
  var geometryColumns = dao.queryForTableName(tableName, function(err, geometryColumns) {
    ...
    this.getFeatureDaoWithGeometryColumns(geometryColumns, callback);
  }.bind(this));
};
 ...
 */
GeoPackage.prototype.queryForGeoJSONFeaturesInTable = function(tableName, boundingBox, callback) {
  var foundFeatures = [];
  this.getFeatureDaoWithTableName(tableName, function(err, featureDao) {
    featureDao.queryForGeoJSONIndexedFeaturesWithBoundingBox(boundingBox, function(err, row, rowCallback) {
      foundFeatures.push(row);
      rowCallback();
    }, function() {
      callback(null, foundFeatures);
    });
  });
}
 ...
 */
GeoPackage.prototype.iterateGeoJSONFeaturesInTableWithinBoundingBox = function(tableName, boundingBox, rowCallback, doneCallback) {
  this.getFeatureDaoWithTableName(tableName, function(err, featureDao) {
    featureDao.queryForGeoJSONIndexedFeaturesWithBoundingBox(boundingBox, rowCallback, doneCallback);
  });
}
...
GeoPackage.prototype.createTileTableWithTableName = function(tableName, contentsBoundingBox, contentsSrsId, tileMatrixSetBoundingBox, tileMatrixSetSrsId, callback) {
  ...
  async.series([
    ...
    function(callback) {
      ...
      callback(null, this.createTileTable(tileTable));
    }.bind(this),
    function(callback) {
      var contents = new Contents();
