...
GeoPackage.prototype.getTileDaoWithTileMatrixSet = function (tileMatrixSet, callback) {
  ...
  tileMatrixDao.queryForAllEqWithField(TileMatrixDao.COLUMN_TABLE_NAME, tileMatrixSet.table_name, null, null, TileMatrixDao.COLUMN_ZOOM_LEVEL + ' ASC, ' + TileMatrixDao.COLUMN_PIXEL_X_SIZE + ' DESC, ' + TileMatrixDao.COLUMN_PIXEL_Y_SIZE + ' DESC')
  .then(function(results) {
...
GeoPackage.prototype.getTileDaoWithTableName = function (tableName, callback) {
  ...
  tms.queryForAllEqWithFieldAndValue(TileMatrixSetDao.COLUMN_TABLE_NAME, tableName)
  .then(function(results) {
    if (results.length > 1) {
...
GeoPackage.prototype.index = function(callback) {
  ...
  async.eachSeries(tables, function(table, tableDone) {
    this.indexFeatureTable(table)
    .then(function(indexed) {
      if (indexed) {
        return callback(null, true);
      } else {
        return callback(new Error('Unable to index table'));
      }
    });
...
GeoPackage.prototype.queryForGeoJSONFeaturesInTable = function(tableName, boundingBox, callback) {
  var foundFeatures = [];
  this.getFeatureDaoWithTableName(tableName)
  .then(function(featureDao) {
    featureDao.queryForGeoJSONIndexedFeaturesWithBoundingBox(boundingBox, function(err, row) {
...
GeoPackage.prototype.createStandardWebMercatorTileMatrix = function(epsg3857TileBoundingBox, tileMatrixSet, minZoom, maxZoom, callback) {
  ...
  async.whilst(
    ...
    function(callback) {
      ...
      callback(null, tileMatrixDao.create(tileMatrix));
    },
    callback
  );
};
...
GeoPackage.prototype.addTile = function(tileStream, tableName, zoom, tileRow, tileColumn, callback) {
  this.getTileDaoWithTableName(tableName, function(err, tileDao) {
    ...
    callback(null, tileDao.create(newRow));
  });
};
...
GeoPackage.prototype.getInfoForTable = function (tableDao) {
  ...
  tableDao.table.columns.forEach(function(column) {
    dcd.getDataColumns(tableDao.table.table_name, column.name)
