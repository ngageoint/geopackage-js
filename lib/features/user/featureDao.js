    ...
    next: function() {
      ...
      if (!nextRow.done) {
        ...
        while(!nextRow.done && !geometry) {
          ...
          geometry = verifyFeature(featureRow, srs, projection, projectedBoundingBox);
          if (geometry) {
  ...
  return {
    ...
    next: function() {
      ...
      if (!nextRow.done) {
        ...
        while(!nextRow.done && !geometry) {
          featureRow = thisGetFeatureRow(nextRow.value);
          geometry = verifyFeature(featureRow, srs, projection, boundingBox);
          if (geometry) {
            var geoJson = {
              properties: {},
              geometry: geometry
...
function verifyFeature(featureRow, srs, projection, boundingBox) {
