...
FeatureDao.prototype.getSrs = function() {
  return this.getGeometryColumnsDao().getSrs(this.geometryColumns);
};
...
FeatureDao.prototype.fastQueryWebMercatorBoundingBox = function(boundingBox, featureRowCallback) {
  var srs = this.getSrs();
  return this.featureTableIndex.queryWithBoundingBox(boundingBox, 'EPSG:3857', function(err, row) {
    var featureRow = this.getFeatureRow(row);
    featureRowCallback(err, featureRow);
  }.bind(this));
}

FeatureDao.prototype.queryIndexedFeaturesWithWebMercatorBoundingBox = function(boundingBox, featureRowCallback) {
  var srs = this.getSrs();
  return this.featureTableIndex.queryWithBoundingBox(boundingBox, 'EPSG:3857', function(err, row) {
    var featureRow = this.getFeatureRow(row);
    var geometry = verifyFeature(featureRow, srs, boundingBox.projectBoundingBox('EPSG:3857', 'EPSG:4326'));
    if (geometry) {
      geometry.properties = featureRow.values;
      return featureRowCallback(err, featureRow);
    }
  }.bind(this));
}
 ...
 */
FeatureDao.prototype.queryForGeoJSONIndexedFeaturesWithBoundingBox = function(boundingBox, geoJSONFeatureCallback) {
  ...
  return this.table.columns.reduce(function(sequence, column) {
    return this.dataColumnsDao.getDataColumns(this.table.table_name, column.name)
  ...
  .then(function() {
    return this.featureTableIndex.queryWithBoundingBox(boundingBox, 'EPSG:4326', function(err, row) {
      ...
      var featureRow = this.getFeatureRow(row);
      var geometry = verifyFeature(featureRow, srs, boundingBox);
      if (geometry) {
        var geoJson = {
          properties: {},
          geometry: geometry
        ...
        return geoJSONFeatureCallback(err, geoJson);
      }
    }.bind(this));
  }.bind(this));
}

function verifyFeature(featureRow, srs, boundingBox) {
...
FeatureDao.prototype.queryIndexedFeaturesWithBoundingBox = function(boundingBox, featureRowCallback) {
  var srs = this.getSrs();
  return this.featureTableIndex.queryWithBoundingBox(boundingBox, 'EPSG:4326', function(err, row) {
    var featureRow = this.getFeatureRow(row);
    var geometry = verifyFeature(featureRow, srs, boundingBox);
