...
GeoPackageAPI.queryForGeoJSONFeaturesInTableFromPath = function(geoPackagePath, tableName, boundingBox, callback) {
  GeoPackageAPI.open(geoPackagePath, function(err, geoPackage) {
    geoPackage.queryForGeoJSONFeaturesInTable(tableName, boundingBox)
...
GeoPackageAPI.iterateGeoJSONFeaturesFromPathInTableWithinBoundingBox = function(geoPackagePath, tableName, boundingBox, rowCallback, doneCallback) {
  GeoPackageAPI.open(geoPackagePath, function(err, geoPackage) {
    geoPackage.iterateGeoJSONFeaturesInTableWithinBoundingBox(tableName, boundingBox, rowCallback)
    .then(function() {
      doneCallback();
    });
  });
...
GeoPackageAPI.iterateGeoJSONFeaturesFromTable = function(geopackage, table, featureCallback, doneCallback) {
  ...
  .then(function(featureDao) {
    if (!featureDao) {
      return doneCallback(new Error('No Table exists with the name ' + table));
    }
    var srs = featureDao.getSrs();
...
GeoPackageAPI.getFeature = function(geopackage, table, featureId) {
  ...
  .then(function(featureDao) {
    ...
    if (feature) {
      currentRow = feature;
      var gj = parseFeatureRowIntoGeoJSON(currentRow, srs);
      return gj;
    } else {
      return featureDao.queryForEachEqWithFieldAndValue('_feature_id', featureId, function(err, feature) {
        if (feature) {
          currentRow = featureDao.getFeatureRow(feature);
        }
      }).then(function() {
        if (!currentRow) {
          return featureDao.queryForEachEqWithFieldAndValue('_properties_id', featureId, function(err, feature) {
            if (feature) {
              currentRow = featureDao.getFeatureRow(feature);
            }
          }).then(function() {
            ...
            return parseFeatureRowIntoGeoJSON(currentRow, srs);
          });
        } else {
          return parseFeatureRowIntoGeoJSON(currentRow, srs);
        }
...
GeoPackageAPI.getTileFromTable = function(geopackage, table, zoom, tileRow, tileColumn, callback) {
  geopackage.getTileDaoWithTableName(table, function(err, tileDao) {
    ...
    tileDao.queryForTile(tileColumn, tileRow, zoom)
    .then(function(tile) {
      callback(null, tile);
    });
  });
