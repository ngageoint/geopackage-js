...
GeoPackageAPI.getFeatureDataAtZoomLevel = function(geopackage, tableName, zoom) {
      var featureData = [];
      var tileDao = geoPackage.getTileDao(tableName);
      var results = tileDao.queryForTilesWithZoomLevel(map.getZoom());
      for(tile of results) {
        var vecTile = new VectorTile.VectorTile(new Pbf(tile.values.tile_data));
