...
GeoPackageAPI.create = function(gppath, callback) {
  ...
  .then(function(geopackage) {
    return geopackage.createSupporteExtensions();
  })
  .then(function(geopackage) {
    if(callback) callback(null, geopackage);
    return geopackage;
...
GeoPackageAPI.iterateGeoJSONFeaturesFromTable = function(geopackage, table) {
  ...
  return {
    ...
    results: featureDao.queryForEach()
  };
};
...
GeoPackageAPI.getFeature = function(geopackage, table, featureId) {
  ...
  if (!feature) {
    var features = featureDao.queryForAllEqWithFieldAndValue('_feature_id', featureId)
    if (features.length) {
      feature = featureDao.getFeatureRow(features[0]);
    } else {
      var features = featureDao.queryForAllEqWithFieldAndValue('_properties_id', featureId)
      if (features.length) {
        feature = featureDao.getFeatureRow(features[0]);
      }
    }
...
GeoPackageAPI.parseFeatureRowIntoGeoJSON = function(featureRow, srs) {
  var geoJson = {
    type: 'Feature',
    properties: {}
  };
...
GeoPackageAPI.getVectorTile = function(geopackage, table, x, y, z) {
  ...
  .then(function(features) {
    ...
    // var vectorTile = new VectorTile.VectorTile(tile);

    // return vectorTile;
    return new VectorTile.VectorTile(new Pbf(vtpbf.fromGeojsonVt(gjvt)));
  });
}
