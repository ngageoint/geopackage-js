...
GeoPackageAPI.getVectorTile = function(geopackage, table, x, y, z) {
  var vtProtobuf = GeoPackageAPI.getVectorTileProtobuf(geopackage, table, x, y, z);
  return vtProtobuf.then(function(vtData) {
    return new VectorTile.VectorTile(new Pbf(vtData));
  });
}
...
GeoPackageAPI.getVectorTileProtobuf = function(geopackage, table, x, y, z) {
  return GeoPackageAPI.getGeoJSONFeaturesInTile(geopackage, table, x, y, z)
...
/**
 * Gets the tile data as a decoded protobuf given an XYZ tile pyramid location
 * @param  {GeoPackage}   geopackage open GeoPackage object
 ...
 */
GeoPackageAPI.getTileDataFromXYZ = function(geopackage, table, x, y, z, width, height) {
  x = Number(x);
  y = Number(y);
  z = Number(z);
  width = Number(width);
  ...
  var bin = retriever.getTileData(x, y, z);

  // This code was written to support a couple of different format possibilities
  return bin.then(function(binData) {
    if(binData) {
      try {
        // This may be a deflated Mapbox VT protobuf. Try to inflate it.
        var result = pako.inflate(binData);
        return new VectorTile.VectorTile(new Pbf(result));
      } catch (err) {
        // The inflate failed, so we will assume that the data is a normal Mapbox VT protobuf
        return new VectorTile.VectorTile(new Pbf(binData));
      }
    }
    // No data was found
    // Giving Pbf an invalid parameter causes it to conveniently return an empty list of layers
    return new VectorTile.VectorTile(new Pbf(binData));
  });
};
...
/**
 * Gets the individual layers in a vector tile table
 * @param {GeoPackage} geopackage   open GeoPackage object
 * @param {String}     tableName    vector tile table
 */
GeoPackageAPI.getLayersInTable = function(geopackage, tableName) {
    var layersDao = geopackage.getVTLayerDao();
    var layersColumns = [];
    for(var i = 0; i < layersDao.table.columns.length; i++) {
        var column = layersDao.table.columns[i];
        layersColumns.push({
            index: column.index,
            name: column.name
        });
    }
    var rowsInTable = layersDao.getRowsForTableName(tableName);
    var layersRows = [];
    for(var i = 0; i < rowsInTable.length; i++) {
        var row = rowsInTable[i];
        row.values = [ row.id, row.table_name, row.name, row.description, row.minzoom, row.maxzoom ];
        layersRows.push(row);
    }
    return {columns: layersColumns, layers: layersRows };
}

/**
 * Gets the properties of all features visible at the current zoom
 * @param  {GeoPackage} geopackage open GeoPackage object
 * @param  {String}     table      name of the feature table
 * @param  {Number}     webZoom    web zoom of displayed map
 * @param  {Number}     west       EPSG:4326 western boundary
 * @param  {Number}     east       EPSG:4326 eastern boundary
 * @param  {Number}     south      EPSG:4326 southern boundary
 * @param  {Number}     north      EPSG:4326 northern boundary
 */
GeoPackageAPI.getFeatureDataAtWebZoom = function(geopackage, table, webZoom, west, east, south, north) {
  var featureData = [];
  ...
  if(tm) {
    ...
    var iterator = tileDao.queryByTileGrid(grid, zoom);
    for (var row of iterator) {
        var vecTile = new VectorTile.VectorTile(new Pbf(row.values.tile_data));
        for(var layerName in vecTile.layers) {
            for(var i = 0; i < vecTile.layers[layerName].length; i++) {
                var feat = vecTile.layers[layerName].feature(i);
                featureData.push(feat.properties);
            }
        }
    }
