 ...
 * @property {Iterable<FeatureRow>} results iterator of results
 */
    ...
    if (geometry.type === 'Point') {
      ...
      if (booleanPointInPolygon(centerPoint, geometry)) {
        ...
        if (line.geometry.type === 'LineString') {
          ...
          line.coordinates.forEach(function(lineString) {
            var distance = pointToLineDistance(centerPoint, lineString);
            if (distance < closestDistance) {
              closest = feature;
              closestDistance = distance;
      ...
      console.log('multi polygon geometry', geometry);
      if (booleanPointInPolygon(centerPoint, geometry)) {
        if (closestDistance !== 0) {
          closest = feature;
          closestDistance = 0;
        ...
        if (line.type === 'FeatureCollection') {
          console.log('feature colleciton', line);
